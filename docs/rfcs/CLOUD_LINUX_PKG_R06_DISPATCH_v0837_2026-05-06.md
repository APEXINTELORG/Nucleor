...
If `release keygen`, signed publish, or package verification is absent or fails,
capture the exact command, stdout/stderr, exit code, and likely source path. Do
not paper over it with a docs-only DONE claim.
