...
- **Status:** WAVE 4 PARTIAL DONE; BROAD DELETION OPEN. `tools/audit_dup_fns.nr`
...
- **Important accounting:** v0838 retired 12 byte-identical non-parser
  ...
  byte-identical `ir_*` helpers, and v0840 Wave 4 retired 25
  byte-identical cache/path/host helpers by moving them into
...
- **Current audit counts after v0840 Wave 4 refresh:** 334
  duplicate function names: 155 `IDENTICAL` safe-delete candidates,
  163 `SIG_MATCH_BODY_DIFFERS` review/replace candidates, and 16
  `SIG_DIFFERS` per-function lift/adapter candidates.
- **Next build item:** delete or import the remaining 155 identical duplicates
