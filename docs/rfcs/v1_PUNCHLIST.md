...
- **Status:** WAVE 3 PARTIAL DONE; BROAD DELETION OPEN. `tools/audit_dup_fns.nr`
...
- **Important accounting:** v0838 retired 12 byte-identical non-parser
  ...
  byte-identical `own_*` ownership helpers, and v0840 retired 25
  byte-identical `ir_*` helpers by moving them into
...
- **Current audit counts after v0840 Wave 3 refresh:** 359
  duplicate function names: 180 `IDENTICAL` safe-delete candidates,
  163 `SIG_MATCH_BODY_DIFFERS` review/replace candidates, and 16
  `SIG_DIFFERS` per-function lift/adapter candidates.
- **Next build item:** delete or import the remaining 180 identical duplicates
