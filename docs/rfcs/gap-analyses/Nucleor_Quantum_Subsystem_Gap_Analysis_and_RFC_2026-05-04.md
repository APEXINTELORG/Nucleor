...
**2026-05-06 helper1 v0836 update:** qsim graph thread safety now has a
runtime guard. `qsim_graph_is_thread_safe()` returns `1`,
`qsim_graph_requires_external_lock()` returns `0`, and all public
`qsim_graph_rt.c` entry points serialize access to the process-local
union-find and gate-DAG arrays with a small C11 atomic spinlock.
`qsim_graph_thread_guard_smoke.nr` runs four concurrent worker threads against
disjoint graph updates and verifies exact final counts. Remaining scalability
gap: per-graph handle state for independent graph ownership rather than one
global process-local graph.
