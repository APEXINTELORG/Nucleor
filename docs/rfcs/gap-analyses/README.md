...
12. **[Tensor / ML / Autodiff](Nucleor_Tensor_ML_Autodiff_Gap_Analysis_and_RFC_2026-05-04.md)** — ML-1 `nuc_attn_flash` ABI mismatch and ML-4 rectangular attention are closed on the current helper branch with direct cross-attention fixture coverage. Tensor rank-2 matmul/transpose/permute, ML-15 P2a int/bool tensor dtype surfaces, quantize FP8/grouped/decode surfaces, ML-5 selective-scan backward, ML-8 NN Conv1D/Conv2D/depthwise forward/backward kernels, ML-9 NN normalization forward/backward, ML-10 causal/encoder-decoder transformer attention, ML-13 P1 XOR plus P2a GNN/P2b SSM/P2c transformer LM-head convergence coverage, and ML-14 Dense plus LayerNorm/BatchNorm/Conv1D/Conv2D/depthwise Conv2D/classic transformer attention autodiff input-gradient bridges now have focused helper evidence. Remaining ML launch risk is full attention-block training, opaque-autodiff, broader SSM backward coverage, Flash/GQA/MLA attention bridge coverage, and GPU depth, not the old flash-attention arity mismatch.
...
14. **[Robotics Control Stack](Nucleor_Robotics_Control_Stack_Gap_Analysis_and_RFC_2026-05-04.md)** — ROBO-1 diagonal TOPP torque-limit tightening, ROBO-2 multi-DOF DMP batch, ROBO-3 planar analytical IK, ROBO-4 6D IK nullspace posture, ROBO-5 latest-two-sample timestamped TF interpolation, ROBO-6 URDF topology parsing + safe serial export/refusal, ROBO-8 CHOMP covariant preconditioning, ROBO-9 planar Cartesian CHOMP, ROBO-10 strict-priority HWBC evidence, ROBO-11 quantum-twin naming mitigation, ROBO-12 magnetometer yaw correction, ROBO-13 fixed-orientation OBB CCD, and ROBO-14 Phase 1 end-to-end smoke coverage exist; ROBO-7 frame-typing safety still Phase A. Mars Climate Orbiter failure mode is live.
...
| **ML-1** | Tensor/ML | CLOSED ON HELPER BRANCH | nuc_attn_flash ABI mismatch — helper1 v0861 fixture calls rectangular flash/GQA paths |
...
| **ROBO-1** | Robotics | PARTIAL | Diagonal TOPP inertia/tau_max acceleration tightening landed; full coupled TOPP-RA dynamics remain |
| **ROBO-3** | Robotics | PARTIAL | Planar 2-link analytical IK landed; 6-DOF canonical manipulator solvers remain |
| **ROBO-4** | Robotics | PARTIAL | 6D IK nullspace posture landed; true 7-DOF manipulator production fixture remains |
...
| **ROBO-6** | Robotics | PARTIAL | URDF topology metadata + safe serial export/refusal landed; true branched-tree FK/runtime and xacro remain |
| **ROBO-8** | Robotics | PARTIAL | CHOMP covariant `A^-1 grad` preconditioning landed; high-DOF / obstacle-gradient evidence and exact SDF gradients remain |
| **ROBO-9** | Robotics | PARTIAL | Planar 2-link Cartesian CHOMP landed; generic high-DOF FK/Jacobian + obstacle/SDF-aware Cartesian gradients remain |
| **ROBO-10** | Robotics | PARTIAL | Strict-priority HWBC evidence landed; box-constrained hierarchy and torque-level / dynamics-coupled control remain |
