...
Update 2026-05-06 helper1 v0872: `nn_batch_norm_ad(input_ad, n_samples, feat_dim, gamma_bits, beta_bits, eps_bits)` now lowers BatchNorm into scalar autodiff tape nodes and registers `nuc_nn_batch_norm` with `differentiable.nr`. `tests/features/nn_autodiff_batch_norm_bridge_smoke.nr` checks value parity against `nn_batch_norm` and input-gradient parity against `nn_batch_norm_backward`. Residual: convolution, attention, opaque registered layer ops, parameter-gradient tape integration, and compiler-side `@differentiable` lowering remain open.

Update 2026-05-06 helper1 v0873: `nn_conv1d_ad(input_ad, kernel_bits, c_in, c_out, length, k_width, stride, pad)` now lowers Conv1D into scalar autodiff tape nodes and registers `nuc_nn_conv1d` with `differentiable.nr`. `tests/features/nn_autodiff_conv1d_bridge_smoke.nr` checks value parity against `nn_conv1d` and input-gradient parity against `nn_conv1d_backward`. Residual: Conv2D, depthwise Conv2D, attention, opaque registered layer ops, parameter-gradient tape integration, and compiler-side `@differentiable` lowering remain open.

Update 2026-05-06 helper1 v0874: `nn_conv2d_ad(input_ad, kernel_bits, c_in, c_out, height, width, k_height, k_width, stride, pad)` now lowers Conv2D into scalar autodiff tape nodes and registers `nuc_nn_conv2d` with `differentiable.nr`. `tests/features/nn_autodiff_conv2d_bridge_smoke.nr` checks value parity against `nn_conv2d` and input-gradient parity against `nn_conv2d_backward`. Residual: depthwise Conv2D, attention, opaque registered layer ops, parameter-gradient tape integration, and compiler-side `@differentiable` lowering remain open.

Update 2026-05-06 helper1 v0875: `nn_depthwise_conv2d_ad(input_ad, kernel_bits, channels, height, width, k_height, k_width, stride, pad)` now lowers depthwise Conv2D into scalar autodiff tape nodes and registers `nuc_nn_depthwise_conv2d` with `differentiable.nr`. `tests/features/nn_autodiff_depthwise_conv2d_bridge_smoke.nr` checks value parity against `nn_depthwise_conv2d` and input-gradient parity against `nn_depthwise_conv2d_backward`. Residual: attention, opaque registered layer ops, parameter-gradient tape integration, and compiler-side `@differentiable` lowering remain open.

Update 2026-05-06 helper1 v0876: `transformer_autodiff.nr` now exposes `tf_attention_split_ad(...)` and `tf_attention_ad(...)`, lowering classic transformer attention into scalar autodiff tape nodes and registering `nuc_tf_attention_split` with `differentiable.nr`. `tests/features/nn_autodiff_attention_bridge_smoke.nr` checks value parity against `tf_attention_split` and `tf_attention_split_masked`, plus Q/V input-gradient coverage on a focused softmax case. Residual: Flash/GQA/MLA attention variants, opaque registered attention ops, parameter-gradient tape integration, and compiler-side `@differentiable` lowering remain open.
...
- ML-14: implement bridge between `autodiff` tape and `nn` rod. Each NN layer becomes a registered op on the autodiff graph. `loss = cross_entropy(nn_forward(x), y); ad_grad(loss, x)` works. **P2a shipped helper1 v0867 for Dense input-gradient composition; P2b/P2c/P2d/P2e/P2f shipped helper1 v0871/v0872/v0873/v0874/v0875 for LayerNorm, BatchNorm, Conv1D, Conv2D, and depthwise Conv2D value/input-gradient composition; P2g shipped helper1 v0876 for classic transformer attention value/input-gradient composition. Opaque registered layer ops, parameter gradients, Flash/GQA/MLA attention bridge coverage, and compiler-side lowering remain open.**
