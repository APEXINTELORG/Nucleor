...
| 2.0.3a | Signature-audit pass — write `tools/audit_dup_fns.nr` (290-line native Nucleor program), enumerate all 429 dup fns, categorize. **Result:** 246 IDENTICAL (57%, safe-delete), 168 SIG_MATCH_BODY_DIFFERS (39%, low-risk replace), 15 SIG_DIFFERS (3.5%, per-fn lift work). Output: `tools/audit_dup_fns_report.csv`. Hard data revises 2.0.3 phasing below. | DONE v0.8.323 — see `findings/promoted/2026-05-06-phase-2-0-3a-dup-fn-audit-results.md` |
| 2.0.3b | Wave 1 — delete 246 IDENTICAL dups from tools_suite + add `import "compiler/nucleor_s1_compiler.nr"`. Audit-driven: every fn in this wave is byte-identical between files, so semantics preserved. Caveat: must include the 168+15 from waves c/d in the same ship to avoid collisions, OR pre-rename via the `_tools_legacy` strategy noted in the finding. | OPEN |
| 2.0.3c | Wave 2 — delete 168 SIG_MATCH_BODY_DIFFERS dups (sigs match → call sites work without modification; tools_suite uses s1's bodies via import). Verify any tools_suite-specific behavior in the deleted bodies isn't lost. | OPEN |
| 2.0.3d | Wave 3 — handle 12 Pattern-A SIG_DIFFERS (s1 canonical, tools_suite stale): delete tools_suite versions + update tools_suite call sites to pass the canonical s1 args. Includes `parse_match_stmt`, `emit_module_ext`, `type_expr` (1739 vs 230 lines), `lower_fn` (476 vs 75 lines), etc. | OPEN |
| 2.0.3e | Wave 4 — handle 3 Pattern-B SIG_DIFFERS (tools_suite canonical, s1 stub): lift `run_build_shared_command`, `run_fix_command`, `run_doc_command` real implementations from tools_suite into s1 (replacing s1's 1-line stubs), then delete tools_suite versions. CLI dispatch in s1 then routes correctly. | OPEN |
