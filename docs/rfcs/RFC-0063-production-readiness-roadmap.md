...
Nucleor paths. The remaining `gen_numerics_matrix.py` is not on the product,
verify, bootstrap, release, or drift path; it is an optional historical matrix
generator and currently lags several committed matrix fixtures.
...
| C2 | `gen_releases_index.py` | 4.7 KB | Native generator shipped; drift gate uses `tools/gen_releases_index.nr` |
| C3 | `gen_rod_manifest.py` | 6 KB | Native generator shipped; drift gate uses `tools/gen_rod_manifest.nr` |
| C4 | `gen_benchmark_summary.py` | 6 KB | Native generator shipped; `docs/BENCHMARK.md` is generated by `tools/gen_benchmark_summary.nr` |
| C5 | `gen_numerics_matrix.py` | 13.7 KB | Optional/offline only. Not drift-gated; current Python generator rewrites 10 committed matrix fixtures and must be refreshed or retired before any native port |
| C6 | `gen_helper_manifest.py` | 35 KB | Native generator shipped; drift gate uses `tools/gen_helper_manifest.nr` while the Python version remains a comparison oracle |
...
2. Output byte-identical to the Python version where the Python version is still
   authoritative, or explicitly document why the Python version is no longer a
   safe oracle.
...
| 5.3 | C5: refresh/retire optional `gen_numerics_matrix.py`; only port after it matches the committed matrix again |
| 5.4 | C6: port `gen_helper_manifest.py` to native (done v0.8.323; Python kept as one-release oracle) |
