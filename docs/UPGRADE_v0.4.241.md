# Upgrading to v0.4.241

**TL;DR — if you're on v0.4.238, v0.4.239, or v0.4.240, upgrade
to v0.4.241 immediately.** Three correctness bugs and one ship-7
diagnostic-quality improvement landed across this window:

- **v0.4.239** fixed `wrapping { ... }` blocks panicking on
  intended overflow under v0.4.238's strict default.
- **v0.4.240** fixed unsigned arithmetic firing signed-overflow
  traps on values near unsigned max.
- **v0.4.241** improved diagnostic carets for `as` casts,
  undefined-fn calls, struct-init field errors, and other type-
  check sites — they now point at the offending token rather
  than the function declaration.

Most other adopters are unaffected. The v0.4.238 default flip
is the only behavior change requiring code review; v0.4.239 and
v0.4.240 are pure bug fixes.

## What changed across v0.4.232 → v0.4.241
...
### Unsigned-overflow trap regression in v0.4.238/v0.4.239 (fixed in v0.4.240)

A second bug exposed by v0.4.238's strict default: the overflow-
intrinsic emitter always emitted **signed**
`@llvm.sadd/ssub/smul.with.overflow.iN` regardless of operand
signedness. Unsigned arithmetic on values where the signed-bit
interpretation would be "negative" (e.g., `u32` values >
2 147 483 647) triggered signed-overflow panics on operations
that wrapped legitimately under unsigned semantics.

You're affected if your code does arithmetic on `u8`, `u16`,
`u32`, or `u64` values near the unsigned max.

**Fix:** upgrade to v0.4.240 or later. v0.4.240 routes
unsigned widths through `uadd/usub/umul.with.overflow.iN`.

### Diagnostic caret improvements (v0.4.241)

Quality-of-life: type-check diagnostics now point at the
offending token instead of the function declaration. Affected
codes: NUM-003 (narrowing cast), NUM-006 (narrow-float vec
read), NUM-024 (cross-width call audit, opt-in), TYP-005
(undefined fn), TYP-008/012/013/017 (struct-init field errors).

This is a pure UX improvement; no code-behavior change. If you
have tooling that parses diagnostic line/col output, you may
want to refresh expected-output snapshots.
...
- **Workaround on v0.4.238:** set
  ...
  codebase).
- **Fix:** upgrade to v0.4.241 or later (v0.4.239 fixes the wrap
  bug; v0.4.241 includes that fix plus the unsigned-overflow fix
  from v0.4.240 and the caret-quality improvements).
...
you're on v0.4.238 (broken) — upgrade to v0.4.241.

## Reference

- Full v0.4.232 → v0.4.241 changelog: `CHANGELOG.md`
