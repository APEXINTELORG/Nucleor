#!/usr/bin/env python3
"""Compare PROBE-2 pipeline 04 (BernoulliNB text classification) Nucleor
stdout against reference JSON's predict fields."""
import json
import re
import sys
...
def main() -> int:
    if len(sys.argv) != 3:
        print("usage: compare_04.py <nucleor_stdout> <reference_json>", file=sys.stderr)
        return 2
    ...
    print(
        f"PROBE-2 04: y_pred byte-equal, accuracy {got['accuracy_correct']}/{got['accuracy_total']}"
    )
    return 0
