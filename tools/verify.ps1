...
Step "Hazard sweep #4: struct fn return + arith + while+format + Vec<Option> + neg" {
    & $bin build "tests/fixtures/t486_hazard_sweep_4_struct_loop_neg.nr" -o "_t486_check" --no-cache 2>&1 | Out-Null
    if (-not (Test-Path "target\_t486_check.exe")) { return $false }
    & "target\_t486_check.exe" | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
}

Step "Hazard sweep #5: method-on-payload + recursive enum + cmp chain + neg lit + empty" {
    & $bin build "tests/fixtures/t487_hazard_sweep_5_recursive_chains.nr" -o "_t487_check" --no-cache 2>&1 | Out-Null
    if (-not (Test-Path "target\_t487_check.exe")) { return $false }
    & "target\_t487_check.exe" | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
}

Step "Hazard sweep #6: div+overflow guards + str_to_int parse + Vec capacity stress" {
    & $bin build "tests/fixtures/t488_hazard_sweep_6_arith_parse_capacity.nr" -o "_t488_check" --no-cache 2>&1 | Out-Null
    if (-not (Test-Path "target\_t488_check.exe")) { return $false }
    & "target\_t488_check.exe" | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
}

Step "Hazard sweep #7: nested struct field a.b.c + arith + sum + HashMap iter + bool field" {
    & $bin build "tests/fixtures/t489_hazard_sweep_7_nested_field_iter.nr" -o "_t489_check" --no-cache 2>&1 | Out-Null
    if (-not (Test-Path "target\_t489_check.exe")) { return $false }
    & "target\_t489_check.exe" | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
}

Step "v0.4.16: mut struct field qualifier + nested format!() type-src" {
    & $bin build "tests/fixtures/t485_mut_field_and_nested_format.nr" -o "_t485_check" --no-cache 2>&1 | Out-Null
    $exe = $null
    if (Test-Path "target\_t485_check.exe") { $exe = "target\_t485_check.exe" }
    elseif (Test-Path "target\_t485_check") { $exe = "target\_t485_check" }
    if (-not $exe) { return $false }
    & $exe | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
...
Step "v0.4.14: scope-local scrutinee inference (format-macro hazard sweep, 3 cases)" {
    # v0.4.14: bounded backward `let X: T` scanner so scrutinee-type
    # lookup doesn't bail on common-name conflicts in big bundles.
    # Plus: ALL-occurrences conflict-detection in
    # infer_pattern_binding_type_from_source.
    & $bin build "tests/fixtures/t483_format_macro_scope_local_inference.nr" -o "_t483_check" --no-cache 2>&1 | Out-Null
    $exe = $null
    if (Test-Path "target\_t483_check.exe") { $exe = "target\_t483_check.exe" }
    elseif (Test-Path "target\_t483_check") { $exe = "target\_t483_check" }
    if (-not $exe) { return $false }
    & $exe | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
...
Step "println of struct field on Some(g) payload binding -- format dispatch" {
    # v0.4.13: pattern-binding fallback in fmt_conversion_for_spec's
    # field-access branch. Pre-fix: println!("{}", g.message) for
    # str field on a Some(g) binding printed the raw pointer.
    & $bin build "tests/fixtures/t482_format_field_access_payload_binding.nr" -o "_t482_check" --no-cache 2>&1 | Out-Null
    $exe = $null
    if (Test-Path "target\_t482_check.exe") { $exe = "target\_t482_check.exe" }
    elseif (Test-Path "target\_t482_check") { $exe = "target\_t482_check" }
    if (-not $exe) { return $false }
    $out = & $exe 2>&1 | Out-String
    if ($out -notmatch "severity=7") { return $false }
    if ($out -notmatch "message=missing capability") { return $false }
    return $true
}
...
Step "Option<MyStruct> with field-access scrutinee — match s.field { Some(g) }" {
    ...
    # Some(g) => g.severity }` typechecks g as GapReport and lowers
    # field access correctly.
    & $bin build "tests/fixtures/t481_option_struct_field_scrutinee.nr" -o "_t481_check" --no-cache 2>&1 | Out-Null
    $exe = $null
    if (Test-Path "target\_t481_check.exe") { $exe = "target\_t481_check.exe" }
    elseif (Test-Path "target\_t481_check") { $exe = "target\_t481_check" }
    if (-not $exe) { return $false }
    & $exe | Out-Null
    if ($LASTEXITCODE -ne 0) { return $false }
    return $true
