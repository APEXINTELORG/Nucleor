...
cli_explain_full_smoke() {
    ...
    # FAST: cache by bin mtime + codes hash. This is a drift gate — if the
    # bin hasn't been rebuilt and the code list hasn't changed, the result
    # is identical, so skip the 150-fork loop entirely.
    local cache_dir="$ROOT/.nuc_cache/verify_explain_full"
    local bin_stamp codes_hash cache_key cache_file
    bin_stamp=$(stat -c '%Y' "$BIN" 2>/dev/null || echo 0)
    codes_hash=$(printf '%s\n' "${codes[@]}" | md5sum | cut -d' ' -f1)
    cache_key="${bin_stamp}_${codes_hash}"
    cache_file="$cache_dir/$cache_key.pass"
    if [ -f "$cache_file" ]; then
        return 0
    fi
    mkdir -p "$cache_dir" 2>/dev/null

    # Spawn each `nuc explain CODE` directly via xargs -P N. Each writes its
    # output to its own file in tmpdir (no shell wrapper per code).
    local tmpdir="$NUC_VERIFY_TMPDIR/cli_explain_full"
    rm -rf "$tmpdir" 2>/dev/null
    mkdir -p "$tmpdir"
    # Use a wrapper script written once + invoked via xargs (single bash spawn
    # decision is at xargs's CreateProcess, not 150 of them inline).
    local worker="$tmpdir/explain_worker.sh"
    cat > "$worker" <<'WEOF'
"$1" explain "$2" > "$3/$2.out" 2>&1
WEOF
    chmod +x "$worker"
    printf '%s\n' "${codes[@]}" | xargs -P 16 -I {} "$worker" "$BIN" "{}" "$tmpdir"
    local fails=()
    local code cause hint
    for code in "${codes[@]}"; do
        local f="$tmpdir/$code.out"
        if [ ! -f "$f" ]; then fails+=("FAIL:$code:no-file"); continue; fi
        if grep -q "unknown error code" "$f"; then fails+=("FAIL:$code:unknown"); continue; fi
        if ! grep -q "$code" "$f"; then fails+=("FAIL:$code:no-self"); continue; fi
        cause=$(sed -n '2p' "$f")
        hint=$(sed -n '3p' "$f")
        [ -z "$cause" ] && fails+=("FAIL:$code:no-cause") && continue
        [ -z "$hint" ] && fails+=("FAIL:$code:no-hint") && continue
    done
    rm -rf "$tmpdir" 2>/dev/null
    if [ ${#fails[@]} -gt 0 ]; then
        printf '       %s\n' "${fails[@]:0:5}"
    ...
    # Mark cache for future runs at same bin+codes.
    rm -f "$cache_dir"/*.pass 2>/dev/null  # purge older entries
    : > "$cache_file"
    return 0
}
