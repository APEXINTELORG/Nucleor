...
helper names. This queue was refreshed after main integration through
`a437f6a6`; start fresh from the fetched current `origin/main`.
...
## Queue 13 Continuation: RFC-0063 Tools-Suite Wave 3

Queue 12 has landed on main as `3bbd8159`, and Cloud PKG/R06 has landed after
it as `dc61411a`. Fetch first and branch fresh from current `origin/main`.
...
## Queue 15 Continuation: RFC-0063 Tools-Suite Wave 5

Queue 14 has landed on main as part of integration commit `2ce9bcba`. Fetch
first and branch fresh from current `origin/main`.

Branch:

```text
fix/helper2-rfc0063-tools-suite-wave5-v0840
```

Scope:

- Continue RFC-0063 Phase 2.0 duplicate deletion/import after Wave 4.
- Select another coherent batch of remaining `IDENTICAL` duplicate helper
  functions.
- Preferred candidates:
  - atomic-ordering cluster: `is_atomic_ordered_builtin`,
    `atomic_tail_order_llvm`, `atomic_cmpxchg_success_order_llvm`,
    `atomic_cmpxchg_failure_order_llvm`, `atomic_rmw_op_llvm`,
    `emit_atomic_ordered_call`, only if confirmed `IDENTICAL`;
  - small map/container cluster: `cmap_new`, `cmap_set`, `cmap_get`,
    `cmap_has`, `ctr_new`, `sym_new`, `sym_set`, `sym_clone`, only if
    confirmed `IDENTICAL`;
  - one dedicated `smap_*` batch only if the selected functions are
    byte-identical and the report explains whether any growth-sensitive helper
    such as `smap_grow` was included.

Hard boundaries:

- Do not touch parser functions: `parse_stmt`, `parse_expr`,
  `parse_match_stmt`.
- Do not touch `SIG_MATCH_BODY_DIFFERS` or `SIG_DIFFERS`.
...
- Push the Wave 5 branch.
- Write `findings/inbox/helper2_rfc0063_tools_suite_wave5_v0840_2026-05-06.md`.
- Include before/after duplicate counts, selected helper names, exact files
  changed, validation output, perf result, and recommended Wave 6 candidates.
...
- Select another coherent batch of 20-40 remaining `IDENTICAL` duplicate helper
  functions.
- Prefer small non-parser clusters with clear command coverage:
  - `smap_*` helpers that are not growth-internal or behavior-sensitive.
  - cache/path helpers such as `cache_v2_*`, `host_*`, `target_root_name`, and
    `write_ll_artifact_if_needed` only if the duplicate audit confirms they are
    truly `IDENTICAL`.
  - small IR accessor/constructor helpers if build-strict and ABI coverage prove
    the move.
- Keep using `compiler/nucleor_rfc0063_shared_wave1.nr` for shared helper
  imports. Do not import all of s1 into tools-suite.

Hard boundaries:

- Do not touch parser functions: `parse_stmt`, `parse_expr`,
  `parse_match_stmt`.
- Do not touch `SIG_MATCH_BODY_DIFFERS` or `SIG_DIFFERS` helpers unless the
  queue stops as a report-only blocker.
- Do not touch Cloud Linux package/R06 tooling or Helper1 ROBO-7 repair.
- Do not change `bin/`, `bootstrap/`, or performance baselines.
- No Python helpers.
...
- Push the Wave 3 branch.
- Write `findings/inbox/helper2_rfc0063_tools_suite_wave3_v0840_2026-05-06.md`.
- Include before/after duplicate counts, selected helper names, exact files
  changed, validation output, perf result, and recommended Wave 4 candidates.
