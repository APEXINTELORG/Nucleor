...
status: CLOSED in v0.4.225. Phase 3c revert in v0.4.224 cleared the bootstrap obstacle that blocked the v0.4.223 fix attempt — with the i32-param-typing gone, the same compile-time fix (detect Option vs Result via expr_full_type_from_sym(qinner) AFTER inner_lx, and dispatch err-tag sentinel: 0 for Result, 1 for Option) now ships clean. Some(x)? correctly extracts x; None? early-returns the whole Option (no OOB at runtime). Regression-guard fixture at tests/features/option_question_op.nr exercises 5 cases (single Some, single None, chained Some*3, chain1 None propagation, chain2 None propagation).
---
